...
from td_cache import get_candles_cached
...
BOT_NAME = os.getenv("BOT_NAME", "AUDUSD_BOT")
...
def fetch_candles(pair, interval, outputsize=120):
    """Cached wrapper around get_candles - see td_cache.py for TTLs."""
    return get_candles_cached(get_candles, pair, interval, outputsize)
...
def build_signal(pair):
    daily_c = fetch_candles(pair, "1day", 60)
    h4_c = fetch_candles(pair, "4h", 120)
    h1_c = fetch_candles(pair, "1h", 160)
    m15_c = fetch_candles(pair, "15min", 160)
    m5_c = fetch_candles(pair, "5min", 200)
...
<b>Mode:</b> Paper Alert Only ({BOT_NAME})
"""
...
def main():
    send_telegram(
        "🤖 <b>AUDUSD Bot V1.0 HTF Narrative ONLINE</b>\n\n"
        "Checklist:\n"
        "Daily Bias → 4H PD Array → 1H PD Array → 15m PD Array → 5m Entry\n\n"
        "Pairs: AUD/USD\n"
        "Mode: Paper alerts only\n"
        "Scan: Every 5 minutes\n"
        f"Risk per trade: EUR {risk_per_trade_eur():.2f}\n"
        "Shared risk tracker: ENABLED (coordinates with GBPUSD bot)"
    )
                ...
                if signal:
                    ...
                    if cooldown_ok or side_changed:
                        ...
                        warn = exposure_warning(
                            BOT_NAME, signal["pair"], signal["side"], trade_risk
                        )
                        if warn:
